...
**Procedure:**
1. Place logic in the right file: `knowledge.routes.ts` / `.service.ts` / `*.repository.ts` / `embedding-provider.ts` / `chunker.ts` / `text-extraction.ts` / `website-extraction.ts`.
...
**Good result looks like:** a real source of the new type reaches `completed`, and a real search query retrieves it, ranked correctly.
