...
| 3 | Redis | **IMPLEMENTED** (local) — already a hard runtime dependency, not a "future" piece | Backs `@fastify/rate-limit` today. Production = managed hosting of the same store — see §3. |
...
| 8 | Client Dashboard | **IMPLEMENTED** | Next.js, session-cookie auth (`requireSession`). Production hosting role is decided; specific vendor is not — see §3. |
...
| 10 | Chat widget | **IMPLEMENTED** | Preact/Vite, Shadow-DOM-isolated, fully runtime-configured (`window.CSAWidgetConfig`) — no rebuild needed per deployment target. |
| 11 | API hosting | **NOT DECIDED** (final vendor) | Role is decided: persistent, container-based hosting (see §3). Render is the *temporary* choice for the immediate widget-testing need, not a production commitment. |
| 12 | Widget static/CDN hosting | **DECIDED FOR PRODUCTION** (target) — **DEFERRED UNTIL NEEDED** (timing, loosely) | Direction: move `widget.js` off the API onto a real static host/CDN. Unlike realtime/jobs, this isn't gated on instance count — it's cheap and low-risk to do at any point; sequencing it is a convenience call, not a technical dependency (see §6). |
| 13 | CI/CD | **IMPLEMENTED** (minimal) | `.github/workflows/ci.yml` runs typecheck, lint, and all three real-DB test suites (`packages/db`, `apps/api`, `apps/widget`) on every push/PR, against Postgres and Redis service containers. Mirrors exactly the steps verified passing locally; not yet exercised by a real GitHub Actions run. Deliberately no deploy step — that stays a separate, later decision. |
| 14 | Dev / staging / production environments | **PARTIALLY IMPLEMENTED** | Dev exists (docker-compose + `.env`). No staging environment exists. "Production" as an environment doesn't exist yet — this document is what it will be built against. |
...
Render is the *immediate, temporary* choice for `apps/api` to unblock real-website widget testing — that is a deployment action tracked separately (see the upcoming Render deployment task), not a production vendor commitment. For final production, only the **role** each piece plays is decided; the vendor is intentionally left open unless a specific vendor is the obviously correct fit for that role.

| Piece | Role | Vendor |
|---|---|---|
| `apps/api` | Persistent, container-based hosting — must support long-lived WebSocket connections and a process that keeps running past an HTTP response (required by §2 until the Redis-backed swaps land, and still true after) | **NOT DECIDED.** Any platform offering a persistent container (not request-scoped serverless/FaaS) qualifies — this is a genuine architectural constraint, not a preference. |
| Client Dashboard (`apps/dashboard`) | Frontend hosting (static/edge-capable Next.js host) | **NOT DECIDED**, though Vercel is a strong natural fit *specifically because* this is a Next.js app built by the same team with zero-config support — noted as the likely default, not locked in. |
| Platform Owner Dashboard | Same deployable as the Client Dashboard today (§1, item 9) — same frontend hosting role | **NOT DECIDED** — whatever hosts the Client Dashboard hosts this too, unless a future decision splits them into separate apps (see §4). |
...
| Redis | Managed Redis, reachable by the API at all times (already a hard dependency today, before pub/sub or a job queue add more reliance on it) | **NOT DECIDED.** |
| `widget.js` | CDN/static hosting, decoupled from the API's own deploys | **DECIDED as a role** (see §1, item 12) — vendor **NOT DECIDED.** |
